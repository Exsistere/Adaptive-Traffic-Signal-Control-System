...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        x1 = Conv2D(16, (4, 4), strides=(2, 2), activation='relu')(input_1)## 16 filters(no.of patterns it will learn) with scans in window of 4x4 at a time
        x1 = Conv2D(32, (2, 2), strides=(1, 1), activation='relu')(x1)## 32 convolusions with 2by2 window
        x1 = Flatten()(x1) ##2D to 1D
        ...
        optimizers = RMSprop(lr=self.learning_rate, clipnorm=1.0)
    ...
    def act(self, state):
        ...
        act_values = self.model.predict([state[0],state[1],state[2]])

        return np.argmax(act_values[0])  # returns action

    def replay(self, batch_size): ##new Q value
        minibatch = random.sample(self.memory, batch_size)
        for state, action, reward, next_state, done in minibatch:
            target = reward
            if not done:
                target = (reward + self.gamma * np.amax(self.model.predict([next_state[0],next_state[1],next_state[2]])[0]))
            print("Q-values before update:", self.target_model.predict([state[0], state[1], state[2]])[0])
            target_f = self.model.predict([state[0],state[1],state[2]])
            print(str(target_f)+": target after")
            target_f[0][action] = target
            self.model.fit([state[0],state[1],state[2]], target_f, epochs=1, verbose=0)
            print("Q-values after update:", self.target_model.predict([state[0], state[1], state[2]])[0])
    ...
    def plot_model(self, file_path="model_plot.png"):
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 700:
            ...
            if(action == 0 and light[0][0][0] == 0):
                ...
                for i in range(10):
                    ...
                    reward1 += traci.edge.getLastStepVehicleNumber('1si') + traci.edge.getLastStepVehicleNumber('2si')
                    reward2 += traci.edge.getLastStepHaltingNumber('3si') + traci.edge.getLastStepHaltingNumber('4si')
                    waiting_time += (traci.edge.getLastStepHaltingNumber('1si') + traci.edge.getLastStepHaltingNumber('2si') + traci.edge.getLastStepHaltingNumber('3si') + traci.edge.getLastStepHaltingNumber('4si'))
                    traci.simulationStep()
        ...
        if e % 1 == 0:
            # Plot the rewards every 10 episodes
        ...
        agent.plot_model("model_plot_episode_" + str(e) + ".png")"""
        ##_build_model
        ...
        agent.save('reinf_traf_control_' + str(e))
        agent.show_weights()
